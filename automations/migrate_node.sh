...
if [[ $action == upgrade ]];then
  for server in `cat ${2}`;do
      ...
      sudo wget --no-check-certificate --progress=bar:force -P /tmp/ https://new_pe_master:8140/packages/2019.8.3/el-7-x86_64/puppet-agent-6.19.1-1.el7.x86_64.rpm &&\
      ...
      sudo sed -i 's/old_pe_master/new_pe_master/g' /etc/puppetlabs/puppet/puppet.conf &&\
  ...
  for server in `cat ${2}`;do
      ...
      sudo wget --no-check-certificate --progress=bar:force -P /tmp/ https://old_pe_master:8140/packages/2018.1.2/el-7-x86_64/puppet-agent-5.5.3-1.el7.x86_64.rpm &&\
      sudo rpm -e puppet-agent &&\
